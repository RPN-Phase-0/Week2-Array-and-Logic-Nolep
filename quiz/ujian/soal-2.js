...
function mengelompokkanAngka(arr) {
  ...
  for (let i = 0; i < arr.length; i++) {
    if (arr[i] % 2 === 0 && arr[i] % 3 !== 0 ) {
        kelompokPertama.push(arr[i]);
    } else if (arr[i] % 2 !== 0 && arr[i] % 3 !== 0) {
        kelompokKedua.push(arr[i]);
    }

    if (arr[i] % 3 === 0) {
        kelompokKetiga.push(arr[i]);
    }
  }

  multidimensi.push(kelompokPertama)
  multidimensi.push(kelompokKedua);
  multidimensi.push(kelompokKetiga)
