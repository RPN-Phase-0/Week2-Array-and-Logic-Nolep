...
function angkaPalindrome(num) {
    while(true){
        num++
        let changeNumberToString = String(num)
        let empty = ''
        for(let i = changeNumberToString.length - 1; i >= 0; i--){
            empty += changeNumberToString[i]
        }
        if(changeNumberToString == empty){
            return changeNumberToString
        }
    }
  }
