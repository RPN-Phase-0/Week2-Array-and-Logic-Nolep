...
for (let i = 0; i < input.length; i++) {
  for (let j = 0; j < input.length; j++) {
    if (j == 0) console.log(`Nama ID: ${input[i][j]}`);
    else if (j == 1) console.log(`Nama Lengkap: ${input[i][j]}`);
    else if (j == 2) console.log(`TTL: ${input[i][j]} ${input[i][j + 1]}`);
    else if (j == 3) console.log(`Hobi: ${input[i][j + 1]}`);
  }
  console.log();
}
